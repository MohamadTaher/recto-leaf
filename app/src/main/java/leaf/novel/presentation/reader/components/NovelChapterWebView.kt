...
private class NovelWebView(context: Context) : WebView(context) {
    ...
    private var chapterBridge: ChapterBridge? = null
    private var chapterBridgeReady = false
    ...
    fun beginChapterLoad(): Int {
        ...
        chapterBridgeReady = false
        return chapterLoadGeneration
    }

    private fun chapterCommand(command: String) {
        pendingChapterCommands.addLast(command)
        flushChapterCommands()
    }

    private fun flushChapterCommands() {
        if (!chapterBridgeReady) return
        while (pendingChapterCommands.isNotEmpty()) {
            val command = JSONObject.quote(pendingChapterCommands.removeFirst())
            evaluateJavascript(
                "window.rectoLeafChapters && window.rectoLeafChapters.command($command);",
                null,
            )
        }
    }
    ...
    fun detachChapterBridge() {
        removeJavascriptInterface(CHAPTER_INTERFACE)
        chapterBridge = null
        chapterBridgeReady = false
    ...
    private class ChapterBridge(
        private val view: NovelWebView,
        private val onMessage: (String) -> Unit,
    ) {
        @JavascriptInterface
        fun postMessage(message: String) {
            view.post { onMessage(message) }
        }
    }
...
fun NovelChapterWebView(
    ...
    val currentOnPastEdge by rememberUpdatedState(onPastEdge)
...
private fun NovelWebView.attachTapDetector(
    ...
    val pastEdgePx = PAST_EDGE_DP * density
    ...
    val detector = GestureDetector(
        ...
        object : GestureDetector.SimpleOnGestureListener() {
            ...
            /** Travel spent scrolling once there is no page left to scroll. */
            private var pastEdgeCarry = 0f

            /** One chapter per gesture: the drag continues over the document that replaces this one. */
            private var pastEdgeSpent = false
            ...
            override fun onScroll(
                ...
                // Scrolling on when the page has run out carries on into the next chapter. Nothing
                // is consumed and nothing resists: it is an ordinary scroll that reached the end,
                // and the small carry is only there so resting against the end does not count.
                if (!pastEdgeSpent && abs(distanceY) > abs(distanceX)) {
                    val forward = distanceY > 0
                    val atEdge = if (forward) scrollOffset >= maxScroll else scrollOffset <= 0
                    if (!atEdge) {
                        pastEdgeCarry = 0f
                    } else {
                        pastEdgeCarry += distanceY
                        if (abs(pastEdgeCarry) >= pastEdgePx) {
                            pastEdgeSpent = true
                            onPastEdge(forward)
                        }
                    }
...
private const val CHAPTER_SECURITY = """
    <meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data: https://novel.rectoleaf.invalid; media-src data: https://novel.rectoleaf.invalid; font-src https://novel.rectoleaf.invalid; style-src 'unsafe-inline' https://novel.rectoleaf.invalid; script-src 'nonce-recto-leaf-reader'">
"""
...
/**
 * Scrolling past the end of the text that carries on into the next chapter.
 *
 * Small on purpose: it is there so that resting against the end of a chapter is not mistaken for
 * asking for the next one, not to make the reader work for the page turn.
 */
private const val PAST_EDGE_DP = 24f
